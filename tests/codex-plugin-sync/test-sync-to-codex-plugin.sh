...
EOF
    ...
    cat > "$repo/assets/claude-code-flow-small.svg" <<'EOF'
...
EOF
    ...
    git -C "$repo" add \
        ...
        .gitignore \
        assets/app-icon.png \
        assets/claude-code-flow-small.svg \
        package.json \
...
write_destination_fixture() {
    ...
    mkdir -p "$repo/plugins/claude-code-flow/skills/example"
    printf 'fixture keep\n' > "$repo/plugins/claude-code-flow/.fixture-keep"
    cat > "$repo/plugins/claude-code-flow/skills/example/SKILL.md" <<'EOF'
# Example Skill

Fixture content.
EOF
    git -C "$repo" add plugins/claude-code-flow/.fixture-keep
    git -C "$repo" add plugins/claude-code-flow/skills/example/SKILL.md
...
add_openai_agent_metadata_fixture() {
    ...
    mkdir -p "$repo/plugins/claude-code-flow/skills/example/agents"

    cat > "$repo/plugins/claude-code-flow/skills/example/agents/openai.yaml" <<'EOF'
interface:
  display_name: "Example"
  short_description: "Destination-owned OpenAI metadata"
EOF

    git -C "$repo" add plugins/claude-code-flow/skills/example/agents/openai.yaml
...
write_synced_destination_fixture() {
    ...
    mkdir -p \
        "$repo/plugins/claude-code-flow/.codex-plugin" \
        "$repo/plugins/claude-code-flow/.private-journal" \
        "$repo/plugins/claude-code-flow/assets" \
        "$repo/plugins/claude-code-flow/skills/example/agents" \
        "$repo/plugins/claude-code-flow/skills/example"

    cat > "$repo/plugins/claude-code-flow/.codex-plugin/plugin.json" <<EOF
{
  "name": "claude-code-flow",
  "version": "$MANIFEST_VERSION"
}
EOF
    ...
    cat > "$repo/plugins/claude-code-flow/assets/claude-code-flow-small.svg" <<'EOF'
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1 1"></svg>
EOF

    printf 'png fixture\n' > "$repo/plugins/claude-code-flow/assets/app-icon.png"
...
EOF
    ...
    printf 'tracked keep\n' > "$repo/plugins/claude-code-flow/.private-journal/keep.txt"

    git -C "$repo" add \
        plugins/claude-code-flow/.codex-plugin/plugin.json \
        plugins/claude-code-flow/assets/app-icon.png \
        plugins/claude-code-flow/assets/claude-code-flow-small.svg \
        plugins/claude-code-flow/skills/example/agents/openai.yaml \
        plugins/claude-code-flow/skills/example/SKILL.md \
        plugins/claude-code-flow/.private-journal/keep.txt
...
write_stale_ignored_destination_fixture() {
    ...
    mkdir -p "$repo/plugins/claude-code-flow/.private-journal"
    printf 'fixture keep\n' > "$repo/plugins/claude-code-flow/.fixture-keep"
    printf 'stale ignored leak\n' > "$repo/plugins/claude-code-flow/.private-journal/leak.txt"
    git -C "$repo" add plugins/claude-code-flow/.fixture-keep
...
main() {
    ...
    dirty_skill_path="$dirty_apply_dest/plugins/claude-code-flow/skills/example/SKILL.md"
    noop_openai_metadata_path="$noop_apply_dest/plugins/claude-code-flow/skills/example/agents/openai.yaml"
    ...
    assert_contains "$preview_section" "assets/claude-code-flow-small.svg" "Preview includes SVG asset"
    ...
    assert_not_contains "$preview_output" "Assets (claude-code-flow-small.svg, app-icon.png) will be seeded from" "Preview omits assets seeding note"
    ...
    assert_branch_absent "$dest" "sync/claude-code-flow-*" "Preview does not create sync branch in destination checkout"
    ...
    assert_contains "$bootstrap_output" "Mode:     BOOTSTRAP (creating plugins/claude-code-flow/ when absent)" "Bootstrap preview describes directory creation"
    ...
    assert_path_absent "$bootstrap_dest/plugins/claude-code-flow" "Bootstrap preview does not create destination plugin directory"
    ...
    assert_branch_absent "$bootstrap_dest" "bootstrap/claude-code-flow-*" "Bootstrap preview does not create bootstrap branch in destination checkout"

    echo ""
    echo "Apply assertions..."
    assert_equals "$dirty_apply_status" "1" "Dirty local apply exits with failure"
    assert_contains "$dirty_apply_output" "ERROR: local checkout has uncommitted changes under 'plugins/claude-code-flow'" "Dirty local apply reports protected destination path"
    ...
    assert_branch_absent "$dirty_apply_dest" "sync/claude-code-flow-*" "Dirty local apply does not create sync branch in destination checkout"
...
Locally modified fixture content." "Dirty local apply preserves tracked working-tree file content"
    ...
    assert_branch_absent "$noop_apply_dest" "sync/claude-code-flow-*" "Clean no-op local apply does not create sync branch in destination checkout"
