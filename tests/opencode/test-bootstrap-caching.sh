...
run_present_file_check() {
    node "$SCRIPT_DIR/test-bootstrap-caching.mjs" "$CCFLOW_PLUGIN_FILE" present
}

run_missing_file_check() {
    mv "$CCFLOW_SKILLS_DIR/using-claude-code-flow/SKILL.md" "$TEST_HOME/using-claude-code-flow.SKILL.md.bak"

    node "$SCRIPT_DIR/test-bootstrap-caching.mjs" "$CCFLOW_PLUGIN_FILE" missing
}
