...
TIMESTAMP=$(date +%s)
OUTPUT_DIR="/tmp/ccflow-tests/${TIMESTAMP}/explicit-skill-requests/claude-describes"
mkdir -p "$OUTPUT_DIR"
...
# Create a plan
cat > "$PROJECT_DIR/.claude/plans/auth-system.md" << 'EOF'
...
claude -p "I have a plan at .claude/plans/auth-system.md. Tell me about my options for executing it, including what subagent-driven-development means and how it works." \
    --model haiku \
