...
Designer produces UI/UX design document based on plan + research → user approval → append to `phase-context.md`. Designer also outputs `.claude/flow/DESIGN.md` (structured component specs, tokens, typography — weaver's primary input). Weaver tasks depend on designer completion in DAG. Standard mode: designer produces a lightweight spec (design direction + component list + key states). Deep/autonomous: full spec with all states, color system, typography, responsive.
