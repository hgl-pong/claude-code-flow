...
For browser-game requests, apply the dynamic browser-game workflow contract embedded in `skills/auto-mode/workflows/full-auto-pipeline.workflow.js`. Default greenfield ambiguous small 2D browser games to prompt-only Phaser + TypeScript + Vite, but preserve explicit user choices and detected React/canvas/plain TypeScript/Three/custom browser or named non-browser runtimes. For explicit 3D/plain TS/Vite prefer vanilla Three.js; for React-hosted 3D prefer React Three Fiber. Do not add repo Phaser/Vite/Three/R3F deps unless the target app plan explicitly requires them.

Plan dependency order where applicable: game-design docs, data-contract, simulation, input map, artist assets, asset manifest/preload, renderer adapter, HUD/UI, playtest evidence. If the spec calls for GAME_DESIGN.md, MECHANICS_SPEC.md, CONTENT_PLAN.md, UX_PLAYTEST_PLAN.md, or ASSET_BRIEF.md, create explicit early planning tasks and make downstream implementation tasks depend on the relevant docs. Asset manifest/preload tasks must depend on real file paths from artist tasks; renderer tasks must depend on verified assets/manifest before consuming file assets.
