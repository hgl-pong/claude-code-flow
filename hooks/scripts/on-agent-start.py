#!/usr/bin/env python
"""SubagentStart: log agent start for duration tracking."""
import json, os, sys
...
def main():
    ...
    agent_name = data.get("name", "") or data.get("agentName", "")
    if not agent_name:
        return
    ...
    entry = {
        ...
        "event": "agent_start",
        "agent": agent_name,
        "model": AGENT_MODELS.get(agent_name, data.get("model", "unknown")),
        "phase": get_phase(),
    }
