...
#
# Sync this claude-code-flow checkout → prime-radiant-inc/openai-codex-plugins.
...
# plugins/claude-code-flow/ when absent, then copies the tracked plugin files from
...
DEFAULT_BASE="main"
DEST_REL="plugins/claude-code-flow"
...
if [[ $BOOTSTRAP -eq 1 ]]; then
  SYNC_BRANCH="bootstrap/claude-code-flow-${UPSTREAM_SHORT}-${TIMESTAMP}"
else
  SYNC_BRANCH="sync/claude-code-flow-${UPSTREAM_SHORT}-${TIMESTAMP}"
fi
...
if [[ $BOOTSTRAP -eq 1 ]]; then
  echo "Mode:     BOOTSTRAP (creating plugins/claude-code-flow/ when absent)"
fi
echo ""
...
if [[ $BOOTSTRAP -eq 1 ]]; then
  COMMIT_TITLE="bootstrap claude-code-flow v$UPSTREAM_VERSION from upstream main @ $UPSTREAM_SHORT"
  PR_BODY="Initial bootstrap of the claude-code-flow plugin from upstream \`main\` @ \`$UPSTREAM_SHORT\` (v$UPSTREAM_VERSION).

Creates \`plugins/claude-code-flow/\` by copying the tracked plugin files from upstream, including \`.codex-plugin/plugin.json\` and \`assets/\`.

Run via: \`scripts/sync-to-codex-plugin.sh --bootstrap\`
Upstream commit: https://github.com/hgl-pong/claude-code-flow/commit/$UPSTREAM_SHA
...
This is a one-time bootstrap. Subsequent syncs will be normal (non-bootstrap) runs using the same tracked upstream plugin files."
  ...
  COMMIT_TITLE="sync claude-code-flow v$UPSTREAM_VERSION from upstream main @ $UPSTREAM_SHORT"
  PR_BODY="Automated sync from claude-code-flow upstream \`main\` @ \`$UPSTREAM_SHORT\` (v$UPSTREAM_VERSION).
...
Upstream: https://github.com/hgl-pong/claude-code-flow/commit/$UPSTREAM_SHA
Branch:   $SYNC_BRANCH"
