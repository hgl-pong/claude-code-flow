...
21. **Routing guard**: if the prompt clearly asks for planning, orchestration, or implementation sequencing, prefer `/plan` over jumping straight into implementation.
...
24. **Built-in plan guard**: do not invoke `EnterPlanMode`. If Claude Code host plan mode is already active, tell the user to exit it and rerun `/plan <task>`.
